...
public class UserSearchController {
    ...
    @ApiOperation("查询搜索记录")
    @GetMapping
    public List<UserSearchDto> get(@Validated UserSearchQueryDto queryDto){
        return userSearchService.queryList(QueryUtil.bulid(queryDto));
    }

    @Log("创建搜索记录")
    @ApiOperation("创建搜索记录")
    @PostMapping
    public UserSearchDto create(@Validated(Insert.class) @RequestBody UserSearchDto dto){
        return userSearchService.create(dto);
    }

    @Log("编辑搜索记录")
    @ApiOperation("编辑搜索记录")
    @PutMapping
    public boolean update(@Validated(Update.class) @RequestBody UserSearchDto dto){
        return userSearchService.update(dto);
    }
    ...
    public boolean delete(@RequestBody Set<Long> ids){
        return userSearchService.deleteByIds(ids);
    }

    @Log("查询用户最近搜索记录")
    @ApiOperation("查询搜索记录")
    @GetMapping("/nearlySearch")
    public List<UserSearchDto> getNearlySearch(@RequestParam(name = "userId") Long userId,
    ...
    public boolean deleteUserAllSearch(@RequestParam Long userId) {
        return userSearchService.remove(Wrappers.<UserSearch>lambdaQuery().eq(UserSearch::getUserId, userId));
    }
}
